...
function( ci_make_lib)
	set( oneValueArgs LIB_NAME APP_NAME CINDER_PATH ASSETS_PATH )
	set( multiValueArgs SOURCES INCLUDES LIBRARIES RESOURCES BLOCKS )

	cmake_parse_arguments( ARG "" "${oneValueArgs}" "${multiValueArgs}" ${ARGN} )

	if( NOT ARG_LIB_NAME )
		set( ARG_LIB_NAME "${PROJECT_NAME}_lib" )
	endif()
	...
	set( CMAKE_RUNTIME_OUTPUT_DIRECTORY ${CMAKE_RUNTIME_OUTPUT_DIRECTORY}/${ARG_LIB_NAME} )

	ci_log_v( "APP_NAME: ${ARG_LIB_NAME}" )
	...
	add_library( ${ARG_LIB_NAME} ${ARG_SOURCES} ${ICON_PATH} ${ARG_RESOURCES} )

	target_include_directories( ${ARG_LIB_NAME} PUBLIC ${ARG_INCLUDES} )
	target_link_libraries( ${ARG_LIB_NAME} cinder ${ARG_LIBRARIES} )

	if( MSVC )
		# Ignore Specific Default Libraries for Debug build
		set_target_properties( ${ARG_LIB_NAME} PROPERTIES LINK_FLAGS_DEBUG "/NODEFAULTLIB:LIBCMT /NODEFAULTLIB:LIBCPMT" )
	endif()
	...
	foreach( block ${ARG_BLOCKS} )
		...
		if( blockName )
			...
			if( TARGET "${blockName}" )
				add_dependencies( ${ARG_LIB_NAME} "${blockName}" )
				target_link_libraries( ${ARG_LIB_NAME} "${blockName}" )
			else()
				...
				if( ${blockName}_INCLUDES )
					target_include_directories( ${ARG_LIB_NAME} PUBLIC ${${blockName}_INCLUDES} )
				endif()
				if( ${blockName}_LIBRARIES )
					target_link_libraries( ${ARG_LIB_NAME} ${${blockName}_LIBRARIES} )
				endif()
			endif()
	...
	if( CINDER_MAC )
		...
		set_target_properties( ${ARG_LIB_NAME} PROPERTIES
			MACOSX_BUNDLE_BUNDLE_NAME ${ARG_LIB_NAME}
